...
object Enemy {
	def spawn(enemyType: String): Enemy = {
		if (enemyType.equals("Seeker")) {
			...
			println(s"Error: Enemy $enemyType cannot be not found, Seeker spawned instead")
			return (new Seeker())
		}
	}
}
...
class Seeker extends Enemy {
	val _position: Position = new Position(math.random * 400, 0)
	val _speed: Double = Const.speed("Seeker")
...
class Bouncer extends Enemy {
	...
	val _speed: Double = Const.speed("Bouncer")
